...
class ConfirmationExecutor:
    ...
    def __init__(self, identity_secret: str,
                 ...
                 adnroid: str, items_confirm: list) -> None:
    ...
    def send_trade_allow_request(self, trade_offer_id: str) -> list:
    ...
    def confirm_sell_listing(self) -> dict:
        ...
        for r in result:
            for i in self._items_confirm:
                if r.name == i.name and r.data_accept == 'Отправить':
                    we_confirm.append(r)
                    break

        data = self._multi_confimm_trans(we_confirm)
        return we_confirm
    ...
    def _multi_confimm_trans(self, we_confirm):
        tag = Tag.ALLOW
        params = self._create_confirmation_params(tag.value)
        params['op'] = tag.value,
