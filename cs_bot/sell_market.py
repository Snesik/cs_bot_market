...
def request_inventory_steam(steam_login_id):
    ...
    all_item = requests.get(f'https://steamcommunity.com/inventory/{steam_login_id}/730/2?l=russian&count=5000',
                            headers=head, timeout=60, proxies=proxies).json()
    return all_item
...
def check_my_price(list_item_id) -> list:
    ...
    if len(request_in_bd) == 1:
        request_in_bd = f'("{request_in_bd[0]}")'
    if request_in_bd:
...
def sell_and_write_in_base(item, price, trader):
            ...
            try:
                ...
                def traders(item) -> None:
                    ...
                    if item.range_price() > 8:
                        sell_and_write_in_base(item, price, trader)
                        return
